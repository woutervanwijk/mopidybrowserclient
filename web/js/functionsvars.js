...
SEARCH_ARTISTS_TABLE = '#artistreaulttable';
SEARCH_SONGS_TABLE = '#songresulttable';
...
function playlisttotable(playlist, table) {
    ...
    with (playlist) {
        for (var i = 0; i < tracks.length; i++) {
            ...
            child += '</td><td><a href="#" class="album" id="' + tracks[i].albumuri + '">' + tracks[i].albumname + '</a></td><td><a href="#" class="time" id="' + tracks[i].uri + '">' + timeFromSeconds(tracks[i].length) + '</a></td></tr>';
            tmp += child;
        };
    }
